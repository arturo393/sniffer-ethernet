...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	res = HAL_ADC_Start_DMA(&hadc1, (uint32_t *) adc_result, ADC_CHANNELS);
	if (res != HAL_OK)
		Error_Handler();
	...
	//-----------------------------------------------------------
		...
		socket_register(buffer, 0x2E, 0x00, (uint8_t*) &_PHYCFGR_RST, sizeof(_PHYCFGR_RST));
		HAL_Delay(500);
		socket_register(buffer, 0x2E, 0x00, (uint8_t*) &_PHYCFGR_NRST, sizeof(_PHYCFGR_NRST));
		HAL_Delay(200);
		...
		socket_register(buffer, 0x1E, 0x01,(uint8_t*) &S_RXBUF_SIZE, sizeof(S_RXBUF_SIZE));
		socket_register(buffer, 0x1F, 0x01,(uint8_t*) &S_TXBUF_SIZE, sizeof(S_TXBUF_SIZE));
		socket_register(buffer, 0x01, 0x01,(uint8_t*) &S_CR_open, sizeof(S_CR_open));
		socket_register(buffer, 0x01, 0x01,(uint8_t*) &S_CR_listen, sizeof(S_CR_listen));
...
//	readWhenDataArrive(s->lora);
	...
	if(read_IR > 0){
		...
		if(s_con > 0){ // SOCK_ESTABLISHED
			socket_register(buffer, 0x01, 0x01,(uint8_t*) &S_CR_con, sizeof(S_CR_con));
		}

		if(s_discon > 0){ //FIN/ACK
			socket_register(buffer, 0x01, 0x01,(uint8_t*) &S_CR_discon, sizeof(S_CR_discon));
			socket_register(buffer, 0x01, 0x01,(uint8_t*) &S_CR_open, sizeof(S_CR_open));
			socket_register(buffer, 0x01, 0x01,(uint8_t*) &S_CR_listen, sizeof(S_CR_listen));
		}
		...
		if(s_recv > 0){
			if(s_send_ok == 0){
				...
				socket_register(buffer, 0x29, 0x01,&s_RX_WR[1], sizeof(s_RX_WR[1]));
				socket_register(buffer, 0x01, 0x01,(uint8_t*) &S_CR_recv, sizeof(S_CR_recv));
				//-------------------------
				point = (s_TX_RD[0] << 8) + s_TX_RD[1];
				socket_register(buffer, point, 0x02,data_reception, len_rx);
				point = point + len_rx;
				q = (point >> 8);
				q = q  & 0x00FF;
				...
				socket_register(buffer, 0x01, 0x01,(uint8_t*) &send_socket0, sizeof(send_socket0));
...
*/
	...
	if(read_IR > 0){
		socket_register(buffer, 0x02, 0x01,(uint8_t*) &ir_reset, sizeof(ir_reset));
		offset_address = 0x02 << 8;
		p = buffer_t;
		memcpy(p, &offset_address, 2);
		transmitir_recibir_spi(buffer_t, 3, &s_IR, sizeof(s_IR));
...
c=0;
	if(c == 1){
		sprintf(re2,re);
		  ...
		  //HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, 1); // NRE485
		  HAL_UART_Transmit(&huart1, dato2, 5, 1000);
		  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_15, 0); // DE485_F232
		  //HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, 0); // NRE485
		  RxXferdif = (huart1.RxXferSize - huart1.RxXferCount);
		  if(RxXferdif != 0){
			  for(uint16_t i=RxXferdif; i>0; i--){
				  huart1.pRxBuffPtr-=1;
			  }
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1|KA_Pin|GPIO_PIN_5|GPIO_PIN_9
                          |GPIO_PIN_12|SPI1_NSS_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, DOUT_Pin|BUSSY_1_Pin|LORA_TX_Pin|NRST_LORA_1_Pin
                          |DIO1_1_Pin|DIO3_1_Pin|GPIO_PIN_6|GPIO_PIN_7, GPIO_PIN_RESET);
